...
import { type Neo4jConfig, createConfig } from "../../src/config/Neo4jConfig";
 ...
 */
export const createTestSession = (driver: Driver, write = false): Session => {
...
export const waitForNeo4j = async (
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		try {
			...
			console.log(
				`Neo4j not ready, attempt ${attempt}/${maxAttempts}:`,
				error.message,
			);
